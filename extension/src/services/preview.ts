...
export class PreviewManager {
  ...
  static async createPreview(args: {
    file: string;
    newContent: string;
    oldContent: string;
  }): Promise<'accept' | 'reject'> {
    const { file, newContent, oldContent } = args;
    ...
    return manager.showDiff(editor, newContent, oldContent);
  }
  ...
  async showDiff(
    ...
    const oldRange = new vscode.Range(
      ...
      editor.document.positionAt(oldContent.length),
    );
